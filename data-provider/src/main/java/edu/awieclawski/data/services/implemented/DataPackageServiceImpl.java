...
class DataPackageServiceImpl implements DataPackageService {
	...
	@Override
	@Transactional
	public void delete(DataPackage data) throws DataNotFoundException {
		List<DataPackage> list = dataPackageRepository.findByUrl(data.getUrl());

		if (list.isEmpty()) {
			log.debug(MSG_DATA_URL_NOT_EXIST, data.getUrl());
			throw new DataNotFoundException(NOT_FOUND_ERR_MSG);
		}

		dataPackageRepository.delete(data);
	...
	@Override
	public List<DataPackage> findAllNotConverted() {
		return dataPackageRepository.findByConvertedFalse();
	}

	@Override
	@Transactional
	public void updateConverted(DataPackage data) {
		List<DataPackage> list = dataPackageRepository.findByUrl(data.getUrl());
	...
	@Override
	@Transactional
	public void updateProcessed(DataPackage data) {
		List<DataPackage> list = dataPackageRepository.findByUrl(data.getUrl());
