...
class NbpRequestServiceImpl implements NbpRequestService {
	...
	List<DataPackage> opPackages, String endPoint, String currencyCode, LocalDate endDate) {
		boolean boundaryElementsFound = rateFacade.findIfExistByCodeAndDateAndType(currencyCode, date, NbpType.A)
		&& rateFacade.findIfExistByCodeAndDateAndType(currencyCode, endDate, NbpType.A);
	...
	throws ConverterException {
		...
		if (Objects.equals(endPoints.aTableRate, endPoint) && endDate == null) {
			entity = dataService.getATypeRatesTableByDateAndSave(date, 0);
			...
			entity = dataService.getATypeRatesTableByDateRangeAndSave(date, endDate, 0);
		} else if (Objects.equals(endPoints.ratesA, endPoint) && endDate != null) {
	...
	throws ConverterException {
		...
		if (codes.isEmpty() || codes.size() > 1) {
			if (Objects.isNull(nbpType) || NbpType.A.equals(nbpType)) {
				...
				endPoints.aTableRate, null, endDate);
			} else if (NbpType.B.equals(nbpType)) {
				return getPackagesAndSaveIfDateBeforeLastExchangeRateDate(date, counter, opPackages,
				endPoints.bTableRate, null, endDate);
			} else if (NbpType.C.equals(nbpType)) {
				return getPackagesAndSaveIfDateBeforeLastExchangeRateDate(date, counter, opPackages,
				endPoints.cTableRate, null, endDate);
			} else {
