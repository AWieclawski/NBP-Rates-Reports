...
public interface RateFacade {
	...
	ExchangeRate findIfExistByCodeDateTable(String code, LocalDate published, String table);

	Boolean findIfExistByCodeAndDateAndType(String code, LocalDate published, NbpType nbpType);

	<T extends ExchangeRateDto> ExchangeRate save(T rate);

	<T extends ExchangeRate> List<T> findAllByType(NbpType nbpType);

	<T extends ExchangeRate> List<T> makeDistinctById(List<T> rates);

	<T extends ExchangeRate> List<T> makeDistinctByTable(List<T> rates);

	<T extends ExchangeRate> List<T> makeDistinctByCurrency(List<T> rates);

	<T extends ExchangeRate> List<Currency> getDistinctCurrencies(List<T> rates);

	void delete(Long id);

	ExchangeRate save(ExchangeRateTypeADto rate, Currency currency);

	<T extends ExchangeRate> List<T> getDemo();
	...
	String tableNo,
	LocalDate date, LocalDate tradingDate) {
